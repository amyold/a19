...
<el-row>
    ...
    <div class='info'>
        ...
        <router-link :to="`/poststation`"  class='in'>驿站中: {{inthepoststation}}</router-link>
         <router-link :to="`/sale`"  class='in'>仓库中: {{inthewarehouse}}</router-link>
         <span class='in' style="color:red">已卖出: {{sale}}</span>
...
data() {
      return {
          ...
          searchList:[
              {
                ordernumber:'abda99',//订单编号
                paymenttime:'2020/12/23',//提交时间
                paystatus:'',//支付状态
                account:30,//用户账户
                orderstatus:'minus',//订单状态,
                ordermoney:'',//订单金额
                condition:'',//物流情况
